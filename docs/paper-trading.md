...
引擎的账户状态（资金、持仓、成交）只存在于进程内存；服务重启或账户重建
（如编排脚本每次启动都会重注册账户）后，`_load_accounts` /
`create_account` 会通过 `_restore_state_from_history` 回放历史委托 CSV
重建状态，避免隔夜持仓丢失、卖出全部废单。

回放规则：

- 只回放已成交委托（`traded_volume > 0`），费用与成交价取 CSV 记录值不重算，
  资金与持仓公式与 `MatchingEngine.match` 一致；
- 每笔回放后与该行记录的 `account_cash` 快照核对（±1 元），不吻合说明该行
  之前发生过历史状态重置（旧版本引擎重启即清零），从该行起重开新段——
  最终只保留最后一段，不产生"幽灵持仓"；
- 同一委托跨日重复写入多个文件时按 `(order_id, order_time, stock_code)`
  去重（`order_id` 是 per-process 计数器，跨会话会撞号）；
- 不重建内存委托列表与委托序号：`_persist_orders` 写当日文件时会合并保留
  当日早前会话的委托行，但跨日只保留各会话自身的行。

`reset_account`（显式重置）不回放，语义保持清零。
