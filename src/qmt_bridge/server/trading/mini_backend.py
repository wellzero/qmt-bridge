...
本类是 ``XtQuantTrader`` 的 1:1 门面（原 XtTraderManager 的实现主体迁移而来），
通过 ``xtquant.xttrader.XtQuantTrader(path, session_id)`` attach 到 miniQMT 客户端。
券商关停 miniQMT 后 ``connect()`` 返回 -1，届时切换 ``--trader-backend bigqmt``。

所有方法签名严格对齐 xtquant.xttrader 的真实 API，支持全部能力位。
"""

import logging

from .backend import CAPABILITY_METHODS

logger = logging.getLogger("qmt_bridge.trading.mini")


class MiniQmtBackend:
    """基于 ``xtquant.xttrader.XtQuantTrader`` 的交易后端（miniQMT 直连）。
    ...
    name = "mini"
    SUPPORTED_CAPABILITIES = frozenset(CAPABILITY_METHODS.keys())
